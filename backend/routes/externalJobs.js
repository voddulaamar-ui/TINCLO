...
router.get('/', async (req, res) => {
  ...
  if (!apiKey) {
    const result = await persistJobs(getMockJobs(), io, query);
  ...
  try {
    ...
    const jobs = (data.data || []).map((job) => ({
      ...
      // JSearch provides a date posted field
      postedAt: job.job_posted_at_datetime_utc || null,
    }));
    res.json(await persistJobs({ jobs, total: jobs.length, source: 'jsearch' }, io, query));
  } catch (error) {
    console.error('JSearch API error:', error.message);
    res.json(await persistJobs(getMockJobs(), io, query));
  }
});
...
async function persistJobs(payload, io, query) {
  try {
    ...
    const jobs = await Promise.all((payload.jobs || []).map(async (job) => {
      ...
      const update = {
        ...
        tags: job.tags || [],
        // Preserve original posted date if available; don't overwrite on updates
        ...(job.postedAt ? { postedAt: new Date(job.postedAt) } : {}),
      };

      // Check if this is a new document (upsert creates it)
      ...
      if (isNew) newJobIds.add(String(saved._id));
      return saved;
    }));

    // Emit socket event for all connected clients whenever jobs are refreshed
    if (io) {
      const newJobs = jobs.filter(j => newJobIds.has(String(j._id)));
      ...
      // For truly new jobs, also send a preference-match notification to online users
      if (newJobs.length > 0) {
        io.emit('jobs:match', {
          title: `🆕 ${newJobs.length} New Job${newJobs.length > 1 ? 's' : ''} Available!`,
          message: `New opportunities posted: ${newJobs.slice(0, 3).map(j => j.title).join(', ')}${newJobs.length > 3 ? '...' : ''}`,
          matchedJobs: newJobs.slice(0, 5),
          icon: '💼',
        });
      }
    }
    ...
    // Still emit a refresh event with raw payload when DB is unavailable
    if (io) {
      io.emit('jobs:new', {
        jobs: payload.jobs || [],
        newCount: 0,
...
function getMockJobs() {
  // Spread mock jobs across varying ages so filters and badges work in demo mode
  const now = Date.now();
